...
def printRatiosFromFig3():
    parameters = {'BW': 10, 'Ant': 4, 'M': 6, 'R': 5/6, 'dt': 0.99, 'df': 1}
    baseBandPower, RFPower, PAPower, OHPower = returnTotalPower(parameters)
    print('Power Amplifier: {:.2f}'.format(PAPower / 759))
    print('OverHead: {:.2f}'.format(OHPower / 297))
    print('Radio: {:.2f}'.format(RFPower / 88))
    print('Baseband: {:.2f}'.format(baseBandPower / 176))
    print('Total: {:.2f}'.format(
        (baseBandPower + RFPower + PAPower + OHPower) / 1320))
